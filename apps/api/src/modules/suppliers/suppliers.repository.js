...
const repo = {
  async findAll({ limit, offset, sort, order, search }) {
    ...
    const r = await query(
      `SELECT s.*, (SELECT COUNT(*) FROM products p WHERE p.supplier_id = s.id) AS product_count
       FROM suppliers s ${where} ORDER BY ${sort === 'name' ? 'name' : 'created_at'} ${order} LIMIT $${p++} OFFSET $${p++}`,
      [...params, limit, offset]
    );
  ...
  async remove(id) { await query(`UPDATE suppliers SET status = 'INACTIVE', updated_at = NOW() WHERE id = $1`, [id]); },
